...
struct csse_332_info {
  ...
  int quantum;
  unsigned long last_run; //jiffies
  unsigned long run_time; //run_time += last_run
};
...
scheduler_fn(void *arg)
{
	...
	while(!kthread_should_stop()) {
		...
		unsigned long last_run_ = (time2 - current_scheduled_process->last_run);
		unsigned long current_run_time = current_scheduled_process->run_time;
		current_scheduled_process->run_time = last_run_ + current_run_time;
		current_scheduled_process->last_run = time2;
		int cur_quant = current_scheduled_process->quantum;

		pr_info("%ld", (current_scheduled_process->run_time * 1000 / HZ));

		if((current_scheduled_process->run_time * 1000 / HZ) > ((cur_quant + 1) * 1000)){ //it has run past its alloted time
			list_del_init(&current_scheduled_process->llist);
			if(cur_quant == 0){
				list_add_tail(&current_scheduled_process->llist, &heads[0]);
			} else {
				list_add_tail(&current_scheduled_process->llist, &heads[cur_quant-1]);
			}

			current_scheduled_process->quantum = cur_quant-1;

			current_scheduled_process->last_run = 0;
			current_scheduled_process->run_time = 0;
			
			pr_info("decreasing quantum of process");
		}
		...
		else { //current_scheduled_process == NULL
			unsigned long time2 = jiffies;
			if(!list_empty(&head3)){
				element = list_first_entry(&head3, struct csse_332_info, llist);

				unsigned long last_run_ = element->last_run;
				unsigned long current_run_time = element->run_time;
				element->run_time = last_run_ + current_run_time;
				element->last_run = time2;
...
static void timer_callback_reset(struct timer_list *timer){
  ...
  mod_timer(&reset_timer, jiffies + msecs_to_jiffies(50000));
}
